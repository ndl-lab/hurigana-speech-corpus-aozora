...
を利用し、音声デイジーデータを音声認識でテキストに変換することで両者の対応を取ることにより音声データとテキストの音声コーパスを構築しています。

更にテキストの漢字の部分に振り仮名を付与して作成したものを公開しています。
...
収集した全作品の情報と作家事の情報のメタデータは本リポジトリの[aozora_metadata.csv](./aozora_metadata.csv)を参照してください。
...
詳細は、2024年3月に開催される「言語処理学会第30回年次大会 (NLP2024)」で、タイトル「音声認識を用いた青空文庫振り仮名注釈付き音声コーパスの構築の試み」で発表予定です。
...
* Whisperによる音声認識は、音声と音声に対応する認識結果がEnd2Endに出力される処理であるため、音素単位での処理を行っていません。
